...
div_df.to_csv('baseline_diversity_indices.csv', index=False)
...
######################
df = pd.read_csv('outputs/sample_counts_w.csv')
...
df.to_csv('estimated_diversity_indices_sample_w.csv', index=False)

#########################
df = pd.read_csv('./analysis_results/sample_counts_w.csv')
...
grouped_df.to_csv('estimated_diversity_indices_plot_w.csv', index=False)
...
grouped_df.to_csv('estimated_diversity_indices_temporal_w.csv', index=False)
#### Diversity
##   Estimates Sys Reg
##
####
...
######################
df = pd.read_csv('outputs/sample_counts_reg.csv')

df['shannon'] = df.apply(shannon_diversity_est, axis=1)
df['simpson'] = df.apply(simpson_diversity_est, axis=1)
df.drop(['0', '1', '2', '3', '4', '5', '6', '7', '8'], axis=1, inplace=True)

df.to_csv('estimated_diversity_indices_sample_reg.csv', index=False)

#########################
df = pd.read_csv('./analysis_results/sample_counts_reg.csv')
...
grouped_df.to_csv('estimated_diversity_indices_plot_reg.csv', index=False)
...
grouped_df.to_csv('estimated_diversity_indices_temporal_reg.csv', index=False)
...
indices.to_csv('outputs/estimated_d_index_temporal_w.csv', index=False)
arr = np.asarray(d_indices)
np.save('outputs/estimated_d_index_temporal_w.npy', arr)
...
for idx, filename in enumerate(filenames):
    ...
    for r in rs:
        ...
        for type_id, row in enumerate(d_index):
            indices = pd.concat([indices, pd.DataFrame({'filename': [filename], 'type_id': [type_id], 'r': r, '0': [row[0]], '1': [row[1]], '2': [row[2]], '3': [row[3]], '4': [row[4]], '5': [row[5]], '6': [row[6]], '7': [row[7]], '8': [row[8]]})])
indices.to_csv('outputs/estimated_d_index_temporal_reg.csv', index=False)
arr = np.asarray(d_indices)
np.save('outputs/estimated_d_index_temporal_reg.npy', arr)